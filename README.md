...
| `/review` | Paranoid staff engineer | Find the bugs that pass CI but blow up in production. Triages CodeRabbit review comments. |
| `/ship` | Release engineer | Sync main, run tests, resolve CodeRabbit reviews, push, open PR. For a ready branch, not for deciding what to build. |
...
[CodeRabbit](https://coderabbit.ai) reviews your PRs automatically with AI. It catches real bugs — race conditions, security issues, things that pass CI and blow up in production.

**Setup:** Install CodeRabbit on your GitHub repo at [coderabbit.ai](https://coderabbit.ai) — it takes about 30 seconds. Once it's reviewing your PRs, gstack picks up its comments automatically. No additional configuration.

The problem with any automated reviewer is triage. CodeRabbit is good, but not every comment is a real issue. Some are false positives. Some flag things you already fixed three commits ago. Without a triage layer, the comments pile up and you start ignoring them — which defeats the purpose.

gstack solves this. `/review` and `/ship` are now CodeRabbit-aware. They read CodeRabbit's comments, classify each one, and take action:
...
The result is a two-layer review: CodeRabbit catches things asynchronously on the PR, then `/review` and `/ship` triage those findings as part of the normal workflow. Nothing falls through the cracks.

It also learns. Every false positive you confirm gets saved to `~/.gstack/coderabbit-history.md`. Future runs auto-skip known FP patterns for your codebase. And `/retro` tracks CodeRabbit's batting average over time — so you can see whether the signal-to-noise ratio is improving.
...
Claude: [syncs main, runs tests, pre-landing review...]
        ...
        CodeRabbit found 3 comments on this PR:
        ...
        Applied advisory lock. Committed: fix: address CodeRabbit
        ...
        Options for the false positive:
          A) Reply to CodeRabbit explaining (recommended)
          B) Fix it anyway
          C) Ignore

You:    A

Claude: Replied to CodeRabbit. Re-running tests after the fix...
        All tests pass. Continuing to version bump.

        [creates PR with CodeRabbit Review section in body]
        PR: https://github.com/you/app/pull/42
```

Three CodeRabbit comments. One real fix. One auto-acknowledged. One false positive pushed back with a reply. Total extra time: about 30 seconds. And the PR body has the full audit trail.
